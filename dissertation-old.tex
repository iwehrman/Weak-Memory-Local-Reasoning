...
\begin{enumerate}
	...
	% \item \emph{definitive value}: if $e \in M$ with $\tp{e} = \tpalloc{\ell}$ then there exists $f \in M$ with $\tp{f} = \tpwrite{\ell,v}$ for some value $v$. 
	% \item \emph{location linearity}: the restriction of the partial order to any location $\ell$, $(M \cap \writesto{\ell}, \restrict{\leq}{\writesto{\ell}})$, is linear;
\end{enumerate}
...
% A consequence of the healthiness conditions is that for every allocated location $\ell \in \alloc{(M,\leq)}$ there is a unique maximum write: $\max_{\leq}(\restrict{M}{\writesto{\ell}})$.
...
\section{Abstract Semantics of Primitive Commands} % (fold)
...
\begin{figure}[h]
	...
	\infax[PC-assign]{(s,h) \pcstep{\cassign{x}{e}} (\recup{s}{x}{\dnexpr{e}s},h)}
	
	\vspace{1em}
	
	\infrule[PC-assume]{\text{if~} \dnbexpr{b}s = \bvt}{(s,h) \pcstep{\cassume{b}} (s,h)}

	\vspace{1em}
	
	\infrule[PC-load]{\text{if~}\dnexpr{e}s \in \alloc{h} \text{~and~} v \in \speek{\dnexpr{e}s}{h}}{(s,h) \pcstep{\cload{x}{e}} (\recup{s}{x}{v},h)}

	\vspace{1em}
	
	\infrule[PC-store]{\text{if~}\dnexpr{e}s \in \alloc{h} \text{~and~} v \in (\writesto{\dnexpr{e}s, \dnexpr{e'}s} \setminus \relset{h})}{(s,h) \pcstep{\cstore{e}{e'}} (s,h \otimes \set{v})}
	
	\vspace{1em}
	
	\infrule[PC-fence]{\text{if~}V \subseteq (\flushes \setminus \relset{h})}{(s,h) \pcstep{\cfence} (s,h \otimes V)}

	\vspace{1em}
	
	\infrule[PC-alloc]{\text{if~}\ell \notin \alloc{h} \text{~and~} v \in (\allocs{\ell} \setminus \relset{h})}{(s,h) \pcstep{\calloc{x}} (\recup{s}{x}{\ell},h \otimes \set{v})}
	
	\vspace{1em}
	
	\infrule[PC-free]{\text{if~}\allocs{\dnexpr{e}s} \cap \relset{h} \neq \nil}{(s,h) \pcstep{\cfree{e}} (s,\restrict{h}{\setevents \setminus (\allocs{\dnexpr{e}s} \cup \writesto{\dnexpr{e}s})})}
...
\begin{figure}[h]
	...
	\infrule[PC-loadA]{\text{if~}\dnexpr{e}s \notin \alloc{h}}{(s,h) \pcstep{\cload{x}{e}} \abort}
	
	\vspace{1em}
	
	\infrule[PC-storeA]{\text{if~}\dnexpr{e}s \notin \alloc{h}}{(s,h) \pcstep{\cstore{e}{e'}} \abort}

	\vspace{1em}
	
	\infrule[PC-freeA]{\text{if~}\allocs{\dnexpr{e}s} \cap \relset{h} = \nil}{(s,h) \pcstep{\cfree{e}} \abort}
...
% section abstract_semantics_of_primitive_commands (end)


\section{Abstract Semantics of Commands} % (fold)
...
\begin{figure}[h]
\centering
			\infrule[C-prim]{(s,h) \pcstep{p} (s',h')}{p,(s,h),\ell \step \cskip,(s',h'),\ell}
			
			\vspace{1em}

			\infrule[C-seq1]{c_1,\sigma,\ell \step c',\sigma',\ell'}{(\cseq{c_1}{c_2}),\sigma,\ell \step (\cseq{c'}{c_2}),\sigma',\ell'}

			\vspace{1em}

			\infax[C-seq2]{(\cseq{\cskip}{c_2}),\sigma,\ell \step c_2,\sigma,\ell}

			\vspace{1em}

			\infax[C-ch1]{(\cchoice{c_1}{c_2}),\sigma,\ell \step c_1,\sigma,\ell}
			
			\vspace{1em}
			
			\infax[C-ch2]{(\cchoice{c_1}{c_2}),\sigma,\ell \step c_2,\sigma,\ell}

			\vspace{1em}
			
			\infax[C-loop]{\cloop{c},\sigma,\ell \step \left(\cchoice{\cskip}{\cseq{c}{\cloop{c}}}\right),\sigma,\ell}

			\vspace{1em}

			\infax[C-lock]{\catomic{c},\sigma,\bvf \step \catomic{c},\sigma,\bvt}
			
			\vspace{1em}
			
			\infrule[C-atom]{c,\sigma,\bvt \step c',\sigma',\bvt}{\catomic{c},\sigma,\bvt \step \catomic{c'},\sigma',\bvt}
			
			\vspace{1em}
	
			\infax[C-unlock]{\catomic{\cskip},\sigma,\bvt \step \cskip,\sigma',\bvf}
...
\begin{figure}[h]
	...
	\infrule[C-primA]{(s,h) \pcstep{p} \abort }{p,(s,h),\ell \step \abort}
			
	\vspace{1em}

	\infrule[C-seqA]{c_1,\sigma,\ell \step \abort}{(\cseq{c_1}{c_2}),\sigma,\ell \step \abort}

	\vspace{1em}

	\infrule[C-atomA]{c,\sigma,\bvt \step \abort}{\catomic{c},\sigma,\bvt \step \abort}
...
\begin{figure}[h]
	...
	\infrule[E-atomA]{c,\sigma,\bvt \rtcl{\step} \abort}{\catomic{c},\sigma \estep \abort}
	
	\vspace{1em}

	\infrule[E-seqA]{c_1,\sigma \estep \abort}{(c_1 \seq c_2),\sigma \estep \abort}
...
% section abstract_semantics_of_commands (end)
...
\section{Concrete Semantics of Primitive Commands} % (fold)
...
% section concrete_semantics_of_primitive_commands (end)

\section{Concrete Semantics of Commands} % (fold)
