...
\end{enumerate}
The first specification asserts that the result loading the value $z$ in memory at address $x$ into variable $t$ result in a state that is otherwise unchanged except that $t = z$. The second specification asserts that the result of storing the value $t+1$ to the address $y$ results in the addition of a new buffered write to the state, which may later commit to memory and overwrite the current, unspecified value. The third specification asserts that the result of explicitly flushing this write to memory with a fence command overwrites the existing value in memory at $y$ with the value $z+1$. 
