...
\subsection{Sequential Commands and Uniprocessor States}
...
To defined these relations formally, we must first define the notion of a uniprocessor machine state. 

\begin{definition}
A \emph{uniprocessor machine state} is a three-tuple $(s,h,B)$, where: \begin{itemize}
...
\end{itemize}
\end{definition} 
The collection of states is written $\setstates$. We sometimes call the components $(h,B)$ a \emph{memory system}, $\mu$, and refer to a state as a pair $(s,\mu)$. We often abuse notation by interchanging memory systems and states in definitions for which the stack is irrelevant. 
...
Uniprocessor assertions denote sets of uniprocessor machine states, and are used to write the pre- and post-conditions of commands in the specification logic. The language is defined by the following grammar: \begin{align*}
    ...
    & \femp \bnfbar \fbaru \bnfbar e \fwriteu e' \bnfbar  \\ 
...
\paragraph{Assertion Abbreviations} The following abbreviation, analogous to the points-to formula of separation logic, describes the result of flushing a single write to memory: \begin{align*}
    e \fpointsto e' \eqdef & e \fwrite e' \conj \fbaru\\
\end{align*} 
