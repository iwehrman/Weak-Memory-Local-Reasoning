...
As in the uniprocessor case, described in Section~\ref{sec:sequential-spatial-separation}, we begin with the simplest notion of separation, \emph{spatial separation}, which describes decompositions of memory systems with disjoint sets of allocated locations. We write $\nu \in \nu_1 \msep \nu_2$ when $\nu$ may be spatially separated in terms of $\nu_1$ and $\nu_2$.\footnote{As before, we write $\nu_1 \mgen \nu_2$ for a separation of generalized memory systems, and later $P_1 \fgen P_2$ for the corresponding separating conjunction. Previously, we also wrote $\mu_1 \sgen \mu_2$ for the more primitive separation of (non-generalized) memory systems, from which the generalized notion of separation was previously derived, but in this chapter we define the generalized notion of separation directly and hence have no need for the more basic operations $\mu_1 \sgen \mu_2$.} 

The formal definition of spatial separation $\nu_1 \msep \nu_2$ is as follows, where $\nu_1 = (h_1,B_1,K_1,\Gamma_1)$ and $\nu_2 = (h_2,B_2,K_2,\Gamma_2)$: \[ \nu_1 \msep \nu_2 \eqdef \setof{(h_1 \uplus h_2, B, K_1 \cup K_2, \Gamma_1 \cup \Gamma_2)}{B \in B_1 \merge B_2 \conj \nu_1 \compat \nu_2},\] where the compatibility relation between generalized multiprocessor memory systems $\nu_1 \compat \nu_2$ is defined as follows: \[ \nu_1 \compat \nu_2 \iffdef \left(\dom{h_1} \cup \dom{B_1} \right) \cap \left( \dom{h_2} \cup \dom{B_2} \right) = K_1 \cap K_2 = \nil.\] 

In both definitions, we lift operations on lists to the analogous operations on functions into lists. That is, the domain of a buffer array $B$ is the union the domains of all the buffers in the array:  \[ \dom{B} \eqdef \bigcup_{i \in \setprocessors} \dom{B(i)},\] and the interleavings of buffer arrays $B_1$ and $B_2$ are buffers arrays for which all buffers are interleavings of their respective buffers in arrays $B_1$ and $B_2$: \[ B \in (B_1 \merge B_2) \iffdef \forall i \in \setprocessors \st B(i) \in B_1(i) \merge B_2(i).\]

It is easy to check that $\nu_1 \msep \nu_2$ yields a set of generalized multiprocessor memory systems. Because the compatibility relation requires disjointness of the domains, $h_1 \cup h_2$ is a function, and clearly $h_1 \cup h_2 = \nil$ if $\Gamma_1 \cup \Gamma_2 = \nil$. 
