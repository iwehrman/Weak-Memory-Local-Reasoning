...
We now give a formal definition of $\safe{n}{c,s,\mu,J,Q}$ by natural number induction on $n$. $\safe{0}{c,s,\mu,J,Q}$ holds always. And for $n \in \setnaturals$, $\safe{n+1}{c,s,\mu,J,Q}$ holds iff the following conditions are true: \begin{enumerate}
	...
	\item For all $\mu_0,\mu_J,\mu_F,s',\mu_1,c'$ such that \begin{enumerate}[(i)]
		\item $\mu_0 \in (\mu_J \fsep (\mu_F \fhash \mu))$,
		\item $\complete{\mu_0}$,
...
\begin{lemma}
	\label{lem:load-safe}
	For all $n \in \setnaturals$ and $\sigma$ such that $\sigma \sentails (e \fwrite{e'} e'') \fsseq P$, if $x \notin \fv{e,e',e'',P}$ then $\safe{n}{\cload{x}{e}_{e'},\sigma,J,\left((e \fwrite{e'} e'') \fsseq P \conj x = e''\right)}$. 
\end{lemma}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item Let $\sigma_0,\sigma_J,\sigma_F$ such that $\sigma_0 \in \sigma_J \fsep (\sigma_F \fhash \sigma)$, $\complete{\sigma_0}$, and either $\locked{\sigma_0}$ or $\sigma_J \sentails J$. We must show that $c,\sigma_0 \nrightarrow \top$. The only aborting step possible is via \textsc{c-prim-a} by way of \textsc{p-load-a}. This requires that $\ext{s}(e) \notin \dom{h \override \funof{B(\ext{s}(e'))}}$. But $\sigma \sentails (e \fwrite{e'} e'') \fsseq P$, which means there exists $\sigma_w$ such that $\sigma_w \sentails (e \fwrite{e'} e'')$ and $\dom{\restrict{\sigma_w}{\ext{s}(e')}} \subseteq \dom{\restrict{\sigma}{\ext{s}(e')}} \subseteq \dom{\restrict{\sigma_0}{\ext{s}(e')}}$. But $\ext{s}(e) \in \dom{\restrict{\sigma_w}{\ext{s}(e')}}$, and so $\ext{s}(e) \in \dom{h \override \funof{B(\ext{s}e')}}$. Hence, the command cannot abort. 

		\item Let $\sigma_0,\sigma_J,\sigma_F,c_1,\sigma_1$ such that $\sigma_0 \in \sigma_J \fsep (\sigma_F \fhash \sigma)$, $\complete{\sigma_0}$, either $\locked{\sigma_0}$ or $\sigma_J \sentails J$, and $c,\sigma_0 \rightarrow c_1,\sigma_1$. We must show $\sigma'_J,\sigma'_F,\sigma'$ such that $\sigma_1 \in \sigma'_J \fsep (\sigma'_F \fhash \sigma')$, $\sigma'_F \taurefines \sigma_F$, either $\locked{\sigma_1}$ or $\sigma'_J \sentails J$, and $\safe{n}{c_1,\sigma',J,((e \fwrite{e'} e'') \fsseq P)}$. The evaluation step is either by \textsc{c-tau} or \textsc{c-prim} by way of \textsc{p-load}. 

		In the case of \textsc{c-tau}, $c_1 = c$ and $\sigma_1 \taurefines \sigma_0$. By Lemma~\ref{lem:separation-tau} there exists $\sigma'_J,\sigma'_F,\sigma'$ such that $\sigma'_J \taurefines \sigma_J$, $\sigma'_F \taurefines \sigma_F$ and $\sigma' \taurefines \sigma$. By Lemma~\ref{lem:flushing-closure}, $\sigma' \sentails (e \fwrite{e'} e'') \fsseq P$ and $\sigma'_J \sentails J$ if not $\locked{\sigma_1}$. It follows from the inductive hypothesis that $\safe{n}{c,\sigma',J,((e \fwrite{e'} e'') \fsseq P)}$. 

		In the case of \textsc{c-prim} and \textsc{p-load}, $c_1 = \cskip$ and if $\sigma_0 = (s,h_0,B_0,k_0)$ and $(h_0 \override \funof{B_0(\ext{s}(e'))})(\ext{s}(e)) = v$ then $\sigma_1 = (\funup{s}{\ptup{x}{v}},h_0,B_0,k_0)$. Let $\sigma'_J = \funup{\sigma_J}{\ptup{x}{v}}$, $\sigma'_F = \funup{\sigma_F}{\ptup{x}{v}}$ and $\sigma' = \funup{\sigma}{\ptup{x}{v}}$. Then $\sigma_1 \in \sigma'_J \fsep (\sigma'_F \fhash \sigma')$ by definition, $\sigma'_J \sentails J$ if not $\locked{\sigma_1}$ because $x \notin \fv{J}$
	\end{enumerate}
