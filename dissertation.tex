...
In the previous chapter, we identified models of assertions with machine states---i.e., the semantics of assertions was given directly in terms of machine states. In this chapter, we generalize the semantics of assertions---and hence also specifications---by describing their semantics in terms of a more elaborate kind of structure: namely, machine state that have been augmented with some additional structure. This additional structure is irrelevant to the behavior of programs, but is required to model an improved assertion logic. 

Informally speaking, it is possible to interpret the machine states that model assertions as merely \emph{partial} descriptions of \emph{complete} states. For example, the precondition of the axiom schema for loading buffered writes \infax[load-buf]{\triple{e \fwriteu e' \fsseq P}{\cload{x}{e}}{(e \fwriteu e' \fsseq P) \conj x = e'}} describes a single write to location $e$, along with some additional writes to other locations described by $P$. Instead of interpreting the axiom as describing the exact precondition for the execution of the load command, one can instead interpret it as describing only the portion of a hypothetical completely described state that is relevant to the operation of the load command. This interpretation is justified by the fact that the frame rules can then be used to infer specifications w.r.t.\ more complete descriptions of the state in a way that preserves truth.  
...
In this section, we shall specifically introduce additional structure to partial states to represent partiality of the write buffers. An additional bit is added to the state structures, which can be informally interpreted as indicating whether or not the ``end'' of the write buffer-part of the state (i.e., the side of the buffer from which writes are flushed to the shared memory) has been completely described, or alternatively whether additional writes may be appended later to the end of the buffer. Alternatively, the buffer-completeness bit can be interpreted as indicating whether or not some writes have flushed from the write buffer to the memory. 
