...
The parallel language differs from the sequential language described in Section~\ref{sec:sequential-programs} in two ways. First, commands may be composed in parallel, indicating that there are no program-order dependencies between the primitives of the constituent commands. Second, each primitive must be annotated with an expression that indicates the processor on which the primitive will be executed. As is discussed below in Section~\ref{sec:commands-and-states}, this will allow us to express both concurrent and interleaving parallelism uniformly. 
