...
\begin{figure}[ht]
    ...
    \]
\end{minipage}}
    \caption{\label{fig:uniprocessor-satisfaction-relation} Sequential satisfaction relation}
\end{figure}
...
Because the spatial separating conjunction $P \fsep Q$ is commutative, there is no need to define its converse operation. The temporal separating conjunction, however, is not. Hence, we define $P \fseqc Q$ as shorthand for $Q \fseq P$, and similarly for $P \fsseqc Q$: \begin{align*}
...
To aid this description, it can help to augment the commands with auxiliary program variables that track the progress of their evaluation. In this case, it helps to augment the sender thread $c_s$ with a variable assignment that indicates that the memory update has taken place. The in the augmented thread $c'_s$ below, $s$ is an auxiliary variable, the assignment to $s$ is considered to be an auxiliary assignment, and that assignment does not appear in the original thread $c_s$:  
\begin{align*}
...
If we are successful in proving these specifications, then we may use an inference rule (\textsc{par}) to derive a specification for the parallel composition of commands whose individual specifications agree to maintain the same invariant, as do the above specifications of $c'_s$ and $c_r$: \[ \infer[\textsc{par}]{\spec{I}{P_s \fsep P_r}{c'_s \oppar c_r}{Q_s \fsep Q_r}}{ \infer{\spec{I}{P_s}{c'_s}{Q_s}}{\vdots} & \infer{\spec{I}{P_r}{c_r}{Q_r}}{\vdots} }\] For space reasons, we abbreviate the pre-conditions of $c'_s$ and $c_r$ as $P_s$ and $P_r$ respectively, and the post-conditions as $Q_s$ and $Q_r$ respectively, repeated below: \begin{align*}
  P_s \eqdef & s = 0 \conj \femp\\
  P_r \eqdef & x = 0 \conj \femp\\
  Q_s \eqdef & s = 1 \conj \femp\\
  Q_r \eqdef & (x = 0 \conj \femp) \disj (x = 1 \conj d \fpointsto 2).
...
\begin{figure}[p]
      ...
      \begin{minipage}{\columnwidth}
            ...
            \infrule[p-load]{\text{if $(h \override \funof{B(i)})(\ext{s}(e)) = v$ and $i \notin K$}}{\cload{x}{e} :(s,h,B,K) \pstep{i} (\funup{s}{\ptup{x}{v}},h,B,K)}

            \vspace{1em}

            \infrule[p-load-a]{\text{if $(h \override \funof{B(i)})(\ext{s}(e)) = \bot$ and $i \notin K$}}{\cload{x}{e} :(s,h,B,K) \pstep{i} \abort}
            
            \vspace{1em}

            \infrule[p-store]{\text{if $\ext{s}(e) \in \dom{h \override \funof{B(i)}}$}}{\cstore{e}{e'} :(s,h,B,K) \pstep{i} (s,h,\funup{B}{\ptup{i}{B(i) \lapp \llit{\ext{s}(e),\ext{s}(e')}}},K)}
            
            \vspace{1em}

            \infrule[p-store-a]{\text{if $\ext{s}(e) \notin \dom{h \override \funof{B(i)}}$}}{\cstore{e}{e'} :(s,h,B,K) \pstep{i} \abort}
            
            \vspace{1em}

            \infrule[p-fence]{\text{if $B(i) = \lnil$}}{\cfence :(s,h,B,K) \pstep{i} (s,h,B,K)}
            
            \vspace{1em}

            \infrule[p-lock]{\text{if $B(i) = \lnil$}}{\clock :(s,h,B,\nil) \pstep{i} (s,h,B,\setprocessors \setminus \set{i})}
            
            \vspace{1em}

            \infrule[p-unlock]{\text{if $B(i) = \lnil$}}{\cunlock :(s,h,B,\setprocessors \setminus \set{i}) \pstep{i} (s,h,B,\nil)}
      \end{minipage}
...
\begin{remark}
    Unlike traditional notions of separation (e.g., as outlined in work on abstract separation logic \cite{DBLP:conf/lics/CalcagnoOY07}), note that temporal separation is not a cancellative operation. That is, it is not the case that the following implication holds: \[ \mu \sseq \mu_0 = \mu \sseq \mu_1 \onlyif \mu_0 = \mu_1.\] A simple counter-example is $\mu = \mu_0 = (1 \mapsto 2, \arnil, \nil)$ and $\mu_1 = (\nil,\arnil,\nil)$. Then $\mu \sseq \mu_0 = \mu = \mu \sseq \mu_1$, but obviously $\mu_0 \neq \mu_1$. 
\end{remark}
...
\begin{figure}[ht]
    ...
    s,\nu & \sentails & P \fseqri P' & \iffdef & \forall \nu_0,\nu_1 \st s,\nu_1 \sentails P' \conj \nu_0 = \nu \mseq \nu_1 \onlyif \\ & & & & \;\; s,\nu_0 \sentails P
      ...
      \]
\end{minipage}}
      \caption{\label{fig:satisfaction-relation} Concurrent satisfaction relation}
\end{figure}
...
\end{align*} The parallel composition rule is then used to derive \[ \spec{J}{x \stackrel{0.5}{\fpointsto} 1 \fsep x \stackrel{0.5}{\fpointsto} 1}{\cload{t}{x}\oppar \cload{u}{x}}{(x \stackrel{0.5}{\fpointsto} 1 \conj t = 1) \fsep (x \stackrel{0.5}{\fpointsto} 1 \conj u = 1)}\] And now the pre-condition of this specification is equivalent to $x \stackrel{1}{\fpointsto} 1$ and the post-condition to $x \stackrel{1}{\fpointsto} 1 \conj t = 1 \conj u = 1$, as desired. 
...
Absent a succeeding barrier, allocation is perfectly natural; it simply adds a new write to an unallocated location to the top of the write buffer: \infax[new]{\spec{J}{\femp}{\cnew{x}{e}_i}{x \stackrel{0}{\fwrite{i}} e}} Note that the write in the post-condition has full permission, which ensures that earlier writes, framed from the left, have distinct locations. Otherwise, this could result in a duplicate allocation.
...
\chapter{Additional Lemmas, Proofs and Conjectures}
\label{ch:proofs}

This chapter contains proofs of various results about the assertion and program logics. Section~\ref{sec:flushing-closure-proofs} describes results about closure properties of the models of the separating conjunctions, and Section~\ref{sec:soundness-proofs} describes results about the soundness of the program logic w.r.t.\ the model given in Chapter~\ref{ch:multiprocessor}. 
...
\begin{lemma}
  \label{lem:spatiotemporal-separation-tau}
...
\begin{proof}
  Without loss of generality we may assume that $\mu = (h,\funup{B}{\ptup{i}{(\ell,v)\lcons b}},K)$ for some $i \notin K$, and thus $\mu' = (\funup{h}{\ptup{\ell}{v}},\funup{B}{\ptup{i}{b}},K)$. Furthermore, we have $h = h_1 \override h_2$, $B(i) = (\ell,v) \lcons b \in B_1(i) \override B_2(i)$ and $K = K_1 \uplus K_2$, assuming $\mu_1 = (h_1,B_1,K_1)$ and $\mu_2 = (h_2,B_2,K_2)$. The least-recent write of $B(i)$, $(\ell,v)$, is either the least-recent write of $B_1(i)$ or $B_2(i)$. 

  In the first case, $B_1(i) = (\ell,v) \lcons b_1'$, and $b \in b'_1 \override B_2(i)$. Let \[\mu'_1 = (\funup{h_1}{\ptup{\ell}{v}},\funup{B_1}{\ptup{i}{b'_1}},K_1).\] Because $i \notin K = K_1 \uplus K_2$, it is also the case that $i \notin K_1$, and so $\mu_1 \taustep \mu'_1$. By definedness of $\mu_1 \shash \mu_2$, we know that if $\ell \in \dom{h_2} \cap \dom{B_1(i)}$ then $i \in K$. Hence $\ell \notin \dom{h_2}$, which means that $\funup{(h_1 \override h_2)}{\ptup{\ell}{v}} = \funup{h_1}{\ptup{\ell}{v}} \override h_2$. It follows that
  \begin{align*}
    \mu' & = (\funup{h}{\ptup{\ell}{v}},\funup{B}{\ptup{i}{b}},K) \\ 
         & = (\funup{h_1}{\ptup{\ell}{v}} \override h_2,\funup{B}{\ptup{i}{b}},K_1 \uplus K_2) \\ 
         & \in (\funup{h_1}{\ptup{\ell}{v}},\funup{B_1}{\ptup{i}{b'_1}},K_1) \shash (h_2,B_2,K_2) \\ 
         & = \mu'_1 \shash \mu_2.
  \end{align*}

  In the second case, $B_2(i) = (\ell,v) \lcons b'_2$, and $b \in B_1(i) \override b'_2$. Let \[\mu'_2 = (\funup{h_2}{\ptup{\ell}{v}},\funup{B_2}{\ptup{i}{b'_2}},K_2).\] Again, $i \notin K_2$, and so $\mu_2 \taustep \mu'_2$. Because $\funup{(h_1 \override h_2)}{\ptup{\ell}{v}} = h_1 \override (\funup{h_2}{\ptup{\ell}{v}})$, it follows that \begin{align*}
    \mu' & = (\funup{h}{\ptup{\ell}{v}},\funup{B}{\ptup{i}{b}},K) \\ 
         & = (h_1 \override (\funup{h_2}{\ptup{\ell}{v}}),\funup{B}{\ptup{i}{b}},K_1 \uplus K_2) \\ 
         & \in (h_1,B_1,K_1) \shash (\funup{h_2}{\ptup{\ell}{v}},\funup{B_2}{\ptup{i}{b'_2}},K_2)\\ 
         & = \mu_1 \shash \mu'_2.
  \end{align*}
\end{proof}
...
\begin{proof}
  Without loss of generality we may assume that $\mu = (h,\funup{B}{\ptup{i}{(\ell,v)\lcons b}},K)$ for some $i \notin K$, and thus $\mu' = (\funup{h}{\ptup{\ell}{v}},\funup{B}{\ptup{i}{b}},K)$. Furthermore, we have $h = h_1 \uplus h_2$, $B(i) = (\ell,v) \lcons b \in B_1(i) \merge B_2(i)$ and $K = K_1 \uplus K_2$, assuming $\mu_1 = (h_1,B_1,K_1)$ and $\mu_2 = (h_2,B_2,K_2)$. The least-recent write of $B(i)$, $(\ell,v)$, is either the least-recent write of $B_1(i)$ or $B_2(i)$. 

  In the first case, $B_1(i) = (\ell,v) \lcons b_1'$, and $b \in b'_1 \merge B_2(i)$. Let \[\mu'_1 = (\funup{h_1}{\ptup{\ell}{v}},\funup{B_1}{\ptup{i}{b'_1}},K_1).\] Because $i \notin K = K_1 \uplus K_2$, it is also the case that $i \notin K_1$, and so $\mu_1 \taustep \mu'_1$. By $\mu_1 \compat_\sepsym \mu_2$ and $\ell \in \dom{\mu_1}$ we have $\ell \notin \dom{h_2}$, which means that $\funup{(h_1 \uplus h_2)}{\ptup{\ell}{v}} = \funup{h_1}{\ptup{\ell}{v}} \uplus h_2$. It follows that
  \begin{align*}
    \mu' & = (\funup{h}{\ptup{\ell}{v}},\funup{B}{\ptup{i}{b}},K) \\ 
         & = (\funup{h_1}{\ptup{\ell}{v}} \uplus h_2,\funup{B}{\ptup{i}{b}},K_1 \uplus K_2) \\ 
         & \in (\funup{h_1}{\ptup{\ell}{v}},\funup{B_1}{\ptup{i}{b'_1}},K_1) \ssep (h_2,B_2,K_2) \\ 
         & = \mu'_1 \ssep \mu_2.
  \end{align*}
...
\begin{proof}
  Without loss of generality we may assume that $\mu = (h,\funup{B}{\ptup{i}{(\ell,v)\lcons b}},K)$ for some $i \notin K$, and thus $\mu' = (\funup{h}{\ptup{\ell}{v}},\funup{B}{\ptup{i}{b}},K)$. Furthermore, we have $h = h_1 \override h_2$, $B(i) = (\ell,v) \lcons b = B_1(i) \lapp B_2(i)$ and $K = K_1 \uplus K_2$, assuming $\mu_1 = (h_1,B_1,K_1)$ and $\mu_2 = (h_2,B_2,K_2)$. The least-recent write of $B(i)$, $(\ell,v)$, is either the least-recent write of $B_1(i)$ or $B_2(i)$. 

  In the first case, $B_1(i) = (\ell,v) \lcons b_1'$, and $b = b'_1 \lapp B_2(i)$. Let \[\mu'_1 = (\funup{h_1}{\ptup{\ell}{v}},\funup{B_1}{\ptup{i}{b'_1}},K_1).\] Because $i \notin K = K_1 \uplus K_2$, it is also the case that $i \notin K_1$, and so $\mu_1 \taustep \mu'_1$. By $\mu_1 \compat_\seqsym \mu_2$, we know that if $\ell \in \dom{h_2} \cap \dom{B_1(i)}$ then $i \in K$. Hence $\ell \notin \dom{h_2}$, which means that $\funup{(h_1 \override h_2)}{\ptup{\ell}{v}} = \funup{h_1}{\ptup{\ell}{v}} \override h_2$. It follows that \begin{align*}
     \mu'_1 \sseq \mu_2 & = (\funup{h_1}{\ptup{\ell}{v}} \override h_2, \funup{B_1}{\ptup{i}{b'_1}} \lapp B_2 ,K_1 \uplus K_2)\\ & = (\funup{(h_1 \override h_2)}{\ptup{\ell}{v}}, \funup{B}{\ptup{i}{b}},K) \\ & = \mu'.
  \end{align*}

   In the second case, $B_2(i) = (\ell,v) \lcons b_2'$, $B_1(i) = \lnil$ and $b = b'_2$. Let \[\mu'_2 = (\funup{h_2}{\ptup{\ell}{v}},\funup{B_2}{\ptup{i}{b'_2}},K_2).\] Because $i \notin K = K_1 \uplus K_2$, it is also the case that $i \notin K_2$, and so $\mu_2 \taustep \mu'_2$. By definition of function overriding, $\funup{(h_1 \override h_2)}{\ptup{\ell}{v}} = h_1 \override \funup{h_2}{\ptup{\ell}{v}}$. It follows that \begin{align*}
      \mu_1 \sseq \mu'_2 & = (h_1 \override \funup{h_2}{\ptup{\ell}{v}}, B_1 \lapp \funup{B_2}{\ptup{i}{b'_2}}, K_1 \uplus K_2)\\ & = (\funup{(h_1 \override h_2)}{\ptup{\ell}{v}}, \funup{B}{\ptup{i}{b}},K) \\ & = \mu'.
  \end{align*}
\end{proof}

\begin{lemma}
  \label{lem:separation-refinement}
  If $\mu \in \mu_1 \sgen \mu_2$, for $\gensym \in \set{(\hashsym),(\sepsym),(\seqsym),(\sseqsym)}$, and $\mu' \taurefines \mu$ then there exists $\mu'_1,\mu'_2$ such that $\mu'_1 \taurefines \mu_1$, $\mu'_2 \taurefines \mu_2$ and $\mu' \in \mu'_1 \sgen \mu'_2$. 
\end{lemma}

\begin{proof}
  By induction on the number of $\tau$ steps from $\mu$ to $\mu'$. The base case is trivial. Otherwise, assume that $\mu'' \taurefines \mu$ and $\mu'' \taustep \mu'$, and by the induction hypothesis that there exists $\mu''_1,\mu''_2$ such that $\mu''_1 \taurefines \mu_1$, $\mu''_2 \taurefines \mu_2$ and $\mu'' \in \mu''_1 \sgen \mu''_2$. In which case the result follows from one of Lemmas~\ref{lem:spatiotemporal-separation-tau},~\ref{lem:spatial-separation-tau} or~\ref{lem:temporal-separation-tau}, and by transitivity of $\taurefines$.
\end{proof}
...
\begin{lemma}
      ...
      If $\safe{n}{c,s,\mu,J,Q}$, $\fv{R} \cap \mod{c} = \nil$, $\defined{\mu_R \shash \mu}$, and $s,\mu_R,\setprocessors \sentails R$, then $\safe{n}{c,s,\mu_R \shash \mu,J,R \fhash Q}$. 
\end{lemma}

\begin{proof}[Sketch]
      By induction on $n$. The base case is trivial. For the induction step, we assume the lemma holds for $n$ and show that it holds for $n+1$. That is, we assume, for any $c,s,\mu,\mu_R,J,Q,R$, that whenever $\safe{n}{c,s,\mu,J,Q}$, $\fv{R} \cap \mod{c} = \nil$, $\defined{\mu_R \shash \mu}$, and $s,\mu_R,\setprocessors \sentails R$, it is also the case that $\safe{n}{c,s,\mu_R \shash \mu,J,R \fhash Q}$ holds. We also assume that $\safe{n+1}{c,s,\mu,J,Q}$, $\fv{R} \cap \mod{c} = \nil$, $\defined{\mu_R \shash \mu}$, and $s,\mu_R,\setprocessors \sentails R$, and show that $\safe{n+1}{c,s,\mu_R \shash \mu,J,R \fhash Q}$. To show this, we must establish three conditions. 

      \begin{enumerate}
            \item Suppose $c = \cskip$. By assumption, $s,\mu_R,\setprocessors \sentails R$, $\defined{\mu_R \shash \mu}$, and if $c = \cskip$ then $s,\mu,\setprocessors \sentails Q$, and so $s,\mu,\setprocessors \sentails Q$. Hence $s,(\mu_R \shash \mu),\setprocessors \sentails R \fhash Q$. 
            ...
            From $\mu'_R \taurefines \mu_R$ and $s,\mu_R,\setprocessors \sentails R$---as well as the assumption $\fv{R} \cap \mod{c} = \nil$ and Lemmas~\ref{lem:step-stcong} and~\ref{lem:assertion-stcong}---it follows that $s',\mu'_R,\setprocessors \sentails R$.
...
\begin{proof}[Sketch]
      Let $s,\mu,\setprocessors \sentails R \fhash P$ and $n \in \setnaturals$. We must show $\safe{n}{c,s,\mu,J,R\fhash Q}$. From $s,\mu,\setprocessors \sentails R \fhash P$, there exists $\mu_R,\mu_P$ with $\mu \in \mu_R \shash \mu_P$ such that $s,\mu_R,\setprocessors \sentails R$ and $s,\mu_P,\setprocessors \sentails P$. By assumption, $\safe{n}{c,s,\mu_P,J,Q}$. The result follows from Lemma~\ref{lem:weak-interleaving-safe}. 
\end{proof}

A proof sketch of the safety of the spatial frame rule relies on the following unproven conjecture. 
\begin{conjecture}
    \label{conj:step-preserves-separation}
    Let $\mu_a \in \mu_b \ssep \mu_c$, and suppose $c,s,\mu_a \step c',s',\mu'_a$. If $\mu'_a \in \mu'_b \shash \mu'c$ with $\mu'_b \taurefines \mu_b$, then $\mu'_b \shash \mu'c = \mu'_b \ssep \mu'c$. 
\end{conjecture}


\begin{lemma}
      \label{lem:spatial-separation-safe}
      If $\safe{n}{c,s,\mu,J,Q}$, $\fv{R} \cap \mod{c} = \nil$, $\defined{\mu_R \ssep \mu}$, and $s,\mu_R,\setprocessors \sentails R$, then $\safe{n}{c,s,\mu_R \ssep \mu,J,R \fsep Q}$. 
\end{lemma}

\begin{proof}[Sketch]
      By induction on $n$. The base case is trivial. For the induction step, we assume the lemma holds for $n$ and show that it holds for $n+1$. That is, we assume, for any $c,s,\mu,\mu_R,J,Q,R$, that whenever $\safe{n}{c,s,\mu,J,Q}$, $\fv{R} \cap \mod{c} = \nil$, $\defined{\mu_R \ssep \mu}$, and $s,\mu_R,\setprocessors \sentails R$, it is also the case that $\safe{n}{c,s,\mu_R \ssep \mu,J,R \fsep Q}$ holds as well. We additionally assume the antecedent, which is that $\safe{n+1}{c,s,\mu,J,Q}$, $\fv{R} \cap \mod{c} = \nil$, $\defined{\mu_R \ssep \mu}$, and $s,\mu_R,\setprocessors \sentails R$, and show the consequent, which is that $\safe{n+1}{c,s,\mu_R \ssep \mu,J,R \fsep Q}$. To show this, we must establish three conditions. 

      \begin{enumerate}
            \item Suppose $c = \cskip$. By assumption, $s,\mu_R,\setprocessors \sentails R$, $\defined{\mu_R \ssep \mu}$, and if $c = \cskip$ then $s,\mu,\setprocessors \sentails Q$, and so indeed $s,\mu,\setprocessors \sentails Q$. Hence $s,\mu_R \ssep \mu,\setprocessors \sentails R \fsep Q$. 

            \item Let $\mu_0,\mu_J,\mu_F$ such that $\mu_0 \in \mu_J \ssep (\mu_F \shash (\mu_R \ssep \mu))$, and either $\locked{\mu_0}$ or $s,\mu_J,\setprocessors \sentails J$. We must show that $c,s,\mu_0 \nrightarrow \abort$. But \begin{align*}
            \mu_J \ssep (\mu_F \shash (\mu_R \ssep \mu)) & \subseteq \mu_J \ssep (\mu_F \shash (\mu_R \shash \mu)) \\ 
            & = \mu_J \ssep ((\mu_F \shash \mu_R) \shash \mu)
            \end{align*} and so by part 2 of assumption $\safe{n+1}{c,s,\mu,J,Q}$, instantiating with (an arbitrary element of) $\mu_F \shash \mu_R$, the result holds. 

            \item Let $\mu_0,\mu_J,\mu_F,\mu_1,c',s'$ such that $\mu_0 \in \mu_J \ssep (\mu_F \shash (\mu_R \ssep \mu))$, either $\locked{\mu_0}$ or $s,\mu_J,\setprocessors \sentails J$, and $c,s,\mu_0 \step c',s',\mu_1$. We must exhibit $\mu'_J,\mu'_F,\mu'$ such that: \begin{enumerate}[(a)]
                  \item $\mu_1 \in \mu'_J \ssep (\mu'_F \shash \mu')$,
                  \item $\mu'_F \taurefines \mu_F$, 
                  \item either $\locked{\mu_1}$ or $s',\mu'_J,\setprocessors \sentails J$, and
                  \item $\safe{n}{c',s',\mu',J,R \fsep Q}$. 
            \end{enumerate}

            We first instantiate part 3 of assumption $\safe{n+1}{c,s,\mu,J,Q}$ with $\mu_J$, (an arbitrary element of) $\mu_F \shash \mu_R$, $\mu_1$, $c'$ and $s'$, which gives us $\mu'_J, \mu'_{FR}, \mu'$ such that: \begin{enumerate}[(a)]
                  \item $\mu_1 \in \mu'_J \ssep (\mu'_{FR} \shash \mu')$
                  \item $\mu'_{FR} \taurefines \mu_F \shash \mu_R$
                  \item either $\locked{\mu_1}$ or $s',\mu'_J,\setprocessors \sentails J$, and
                  \item $\safe{n}{c',s',\mu',J,Q}$.
            ...
            Next, we wish to apply the inductive hypothesis to $\safe{n}{c',s',\mu',J,Q}$ to show that $\safe{n}{c',s',\mu'_R \ssep \mu',J,R \fsep Q}$. This follows from the observations that $\fv{R} \cap \mod{c'} = \nil$ (because $\mod{c'} \subseteq \mod{c}$ from Lemma~\ref{lem:step-stcong}) and $\defined{\mu'_R \ssep \mu'}$. 

            Returning to our original task, we exhibit $\mu'_J$, $\mu'_F$ and $\mu'_R \ssep \mu'$. By Conjecture~\ref{conj:step-preserves-separation} it follows that \[ \mu_1 \in \mu'_J \ssep (\mu'_F \shash (\mu'_R \ssep \mu')).\] We have already showed that $\mu'_F \taurefines \mu_F$. Either $\locked{\mu_1}$ or $s',\sigma'_J,\setprocessors \sentails J$ by assumption. Finally $\safe{n}{c',s',\mu'_R \ssep \mu',J,R \fsep Q}$ by the inductive hypothesis above. 
      \end{enumerate}
\end{proof}
...
\begin{thesisauthorvita}
    Ian Wehrman was born in St.\ Louis, Missouri on June 7, 1980. He received his B.Sc.\ in Computer Science from Webster University in 2004 and his M.Sc.\ in Computer Science from Washington University in St.\ Louis in 2006. His thesis describes the first fully automated approach to solving the word problem from automated deduction by using termination checkers of term-rewriting systems. Later in 2006, he entered the doctoral program in Computer Science at the University of Texas at Austin, where he studied formal methods, concurrency and the theory of programming languages. He received a Ph.D in 2012 for a dissertation that describes a program logic for local reasoning about the behavior of programs w.r.t.\ a weak, x86-like memory model. 
