...
Consider the following trivial sequential program $c$, which stores the value 1 into an address $x$, flushes that value to memory with a fence instruction, and then loads the value in memory back from $x$ into variable $y$: \[ c \eqdef \cseq{\cstore{x}{1}}{\cseq{\cfence}{\cload{y}{x}}}.\] This sequential program will fail with a memory error if the value of $x$ is not allocated memory address; any provable specification of this program must require this from the beginning. If the value of $x$ is indeed allocated, then upon termination the buffer will be empty, the value in memory at location $x$ will be 1, and the variable $y$ will also have value 1. In the program logic developed in this chapter, we express this as the following specification: \[ \triple{x \fpointsto -}{c}{x \fpointsto 1 \conj y = 1}.\]
We write $x \fpointsto 1$ to indicate that the value of $x$ is an allocated memory address, and that the value of memory at that address is $1$. Before the program is executed, when the value is left unspecified, we write $x \fpointsto -$. 
...
To complete the proof, we must find intermediate assertions $R_1$ and $R_2$ for which the leaves of the proof sketch above can be derived. For the first command, note that store command $\cstore{x}{1}$ \emph{buffers} a new write, that may be flushed to memory later. We write $x \fwriteu 1$ to indicate a buffered write to memory address $x$ with value 1, and $x \fpointsto - \fseq x \fwriteu 1$ to describe the \emph{temporal composition} of $x \fpointsto -$ and $x \fwriteu 1$, where the left-hand side precedes the right-hand side. The first leaf of the proof is thus: 
\[ 
\infer[\textsc{store}]{\triple{x \fpointsto -}{\cstore{x}{1}}{x \fpointsto - \fseq x \fwriteu 1}}{}
\] which is derivable by the \textsc{store} axiom. 

For the second leaf, we must find $R_2$ such that \[ \triple{x \fpointsto - \fseq x \fwriteu 1}{\cfence}{R_2}.\] The fence command flushes any buffered writes to main memory. In this case, the result of flushing the buffered write yields $x \fpointsto 1$. Hence, we wish specifically to derive: \[ \triple{x \fpointsto - \fseq x \fwriteu 1}{\cfence}{x \fpointsto 1}. \] The axiom for the fence command allows us to additively conjoin to any precondition $P$ an assertion $\fbaru$ that ensures all writes have flushed. Hence, we may derive $(x \fpointsto - \fseq x \fwriteu 1) \conj \fbaru$. But this assertion is logically equivalent to $x \fpointsto 1$, and so we may use the \emph{rule of consequence} to complete this leaf of the proof tree as follows:
\[ 
...
It remains to show $\triple{x \fpointsto 1}{\cload{y}{x}}{x \fpointsto 1 \conj y = 1},$ but this, as we shall see, is an instance of the \textsc{load-mem} axiom scheme: \[ \infer[\textsc{load-mem}]{\triple{x \fpointsto 1}{\cload{y}{x}}{x \fpointsto 1 \conj y = 1}}{}\] which completes the derivation of third leaf of the proof tree, and hence completes the derivation of the example specification. 
...
From the point of view of the individual commands $c_w$ and $c_r$, however, the memory at addresses $r$ and $d$ must be considered shared, because neither command make sole use of that memory; both addresses are accessed by both commands. Hence, to prove a specification of command individually, we must express the relationship between $r$ and $d$ as an invariant, and show that both commands maintain that invariant. Informally, the invariant is that whenever $r \fpointsto 1$ then also $d \fpointsto 1$. Formally, we might express this (without implication) with the following invariant assertion $I$: \[  I \eqdef \exists r' \st \exists d' \st r \fpointsto r' \fsep d \fpointsto d' \fsep (!r' \disj d'). \] This means that $r$ and $d$ are always valid addresses and, referring to current values at those addresses as $r'$ and $d'$ respectively, it is always the case that if $r'$ is 1 then so is $d'$. We might then try to prove the following stronger specification: \[ \spec{I}{\femp}{c_w \oppar c_r}{!x \disj y}.\] The originally desired specification would then follow from an application of the rule of consequence as well as an inference rule (\textsc{share}) which states that, if a specification with invariant $J$ is true, so is the specification with $J$ locally true in the precondition and locally true in the postcondition; i.e., if $J$ holds always, then surely it also holds at the beginning and the end of execution: \[ \infer[\textsc{cons}]{\spec{\femp}{r \fpointsto 0 \fsep d \fpointsto -}{c_w \oppar c_r}{!x \disj y}}{
  \infer[\textsc{share}]{\spec{\femp}{I \fsep \femp}{c_w \oppar c_r}{I \fsep (!x \disj y)}}{\infer{\spec{I}{\femp}{c_w \oppar c_r}{!x \disj y}}{\vdots}}
}\] Here, the rule of consequence may be used because of the following semantic entailments: \begin{align*}
    r \fpointsto 0 \fsep d \fpointsto - \sentails \, &\exists r' \st \exists d' \st r \fpointsto r' \fsep d \fpointsto d' \fsep (!r' \disj d') \fsep \femp \\ 
    &\exists r' \st \exists d' \st r \fpointsto r' \fsep d \fpointsto d' \fsep (!r' \disj d') \fsep (!x \disj y)\sentails\, !x \disj y
...
Next, we must show $\spec{I}{\femp}{c_w \oppar c_r}{!x \disj y}$. We use the rule of consequence and a structural inference rule (\textsc{par}) for the parallel composition of commands, which allows us to infer a combined specification from specifications of individual threads when their local states are spatially disjoint and their invariants match: \[ \infer{\vdots}{\infer[\textsc{cons}]{\spec{I}{\femp}{c_w \oppar c_r}{!x \disj y}}{\infer[\textsc{par}]{\spec{I}{\femp \fsep \femp}{c_w \oppar c_r}{\femp \fsep (!x \disj y)}}{\infer{\spec{I}{\femp}{c_w}{\femp}}{\vdots} & \infer{\spec{I}{\femp}{c_r}{!x \disj y}}{\vdots}}}}\]   

It remains to show that the sequential commands $c_w$ and $c_r$ satisfy their individual specifications, each maintaining the invariant $I$ along the way. In the case of the writing thread, we start with an axiom for the $\clock$ command: \[ \spec{I}{\femp}{\clock_0}{\femp \fhash \flock{0}}\] We then use a rule (\textsc{inv}) for accessing the shared invariant while holding the lock to show that the successive store commands maintain the invariant: \[ \infer[\textsc{inv}]{\spec{I}{\femp \fhash \flock{0}}{\cstore{d}{1}_0 \opseq \cstore{r}{1}_0}{\femp \fhash \flock{0}}}{\infer{\spec{\femp}{I \fsep \flock{0}}{\cstore{d}{1}_0 \opseq \cstore{r}{1}_0}{I \fsep \flock{0}}}{\vdots}}\] The proof is then finished with an axiom for the unlock command: \[ \spec{I}{\femp \fhash \flock{0}}{\cunlock_0}{\femp} \]
...
The satisfaction relation is defined by recursion on the structure of $P$ below in Figure~\ref{fig:satisfaction-relation}, which makes use of the following auxiliary definitions: \begin{align*}
  ...
  \mathit{lock}(i) \eqdef & \setof{(h,B,k)}{h = \nil \conj (\forall j \st B(j) = \lnil) \conj k = \setprocessors \setminus \set{i}} \\ 
  \mathit{kernel}(S) \eqdef & \setof{\mu \in S}{\forall \mu' \taurefines \mu \st \mu' \in S}\\  
  \mathit{trim}(h,B,k) \eqdef & (h,(\lambda x .\lnil),k)\\  
  \mathit{exp}(\mu) \eqdef & \mathit{kernel}(\mathit{trim}^{-1}(\mu))\\ 
  \mathit{pending}(i,\ell,v) \eqdef & \setof{(h,B,k)}{h = \nil \conj (\forall j \st j = i \disj B(j) = \lnil) \conj B(i) = \llit{(\ell,v)} \conj k \subseteq \set{i}} \\ 
  \mathit{flushed}(\ell,v) \eqdef & \setof{(h,B,k)}{h = \ell \mapsto v \conj (\forall j \st j \neq i \onlyif B(j) = \nil) \conj k = \nil} 
\end{align*}
...
\begin{figure}[ht]
    ...
    (s,h,B,k) & \sentails & \fexp{P} & \iffdef & \forall h',B' \st (h',B',k)\taurefines (h,B,k) \onlyif \\ & & & & (s,h',(\lambda x . \lnil),k) \sentails P \\ 
...
\section{A Smaller Load Axiom} 
Right-hand temporal frame rule and the top predicate. 
