...
We might like to prove such a specification: i.e., assuming $d \fpointsto 0$ holds initially, that the composition is safe and and that $d \fpointsto (x+1)$ holds upon termination: \begin{align}
...
We may then try to prove the following constituent specifications, which maintain the invariant $I$: \begin{align*}
  ...
  &\spec{I}{x = 0 \fsep \femp}{c_r}{(x = 0 \fsep \femp) \disj (x = 1 \fsep d \fpointsto 1)}
...
s = 0 \fsep x = 0 \fsep d \fpointsto 0 & \sentails I \fsep P_s \fsep P_r \\ 
...
\Calc{
  ...
  \conn{}{$(x = 0 \fsep \femp) \disj (x = 1 \fsep d \fpointsto 1)$}

  $\mathsf{if} (x)$

  \conn{}{$x = 1 \fsep d \fpointsto 1$}

  $\cstore{d}{2}_1$

  \conn{}{$(x = 1 \fsep d \fpointsto 1) \fseq d \fwrite{1} 2$}

  $\cfence_1$

  \conn{}{$d \fpointsto (x + 1)$}
