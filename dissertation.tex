...
\chapter{Barrier as Resource: A Digression}

In this section we briefly discuss a generalization of the sequential program logic described previously in this chapter. In particular, we refine the semantics of the $\fbaru$ assertion. Immediately, the changes we introduce will allow for a simpler sequential proof theory. They also constitute a necessary generalization for the multiprocessor model and concurrent program logic described in the next chapter. But because is possible to discuss the generalization in sequential setting, we shall do so for the sake of simplicity. 

\section{Partial and Complete States}

It is possible to interpret memory systems and states as described in the previous chapter as \emph{partial} descriptions of \emph{complete} states. For example, the precondition of one axiom for the load command: \infax[load-buf]{\triple{e \fwriteu e' \fsseq P}{\cload{x}{e}}{(e \fwriteu e' \fsseq P) \conj x = e'}} which describes a single write to location $e$, and possibly some additional writes to other locations, can be interpreted as describing only the part of a complete state that is relevant to the operation of the load command. The frame rules can then be used to infer specifications w.r.t.\ more complete descriptions of the state. 

It can be convenient to make formal the distinction the partial and complete states. This allows us to add some additional structure to the representation of states, which can enable a more flexible definition of the semantics of assertions, while the semantics of programs and their specifications may be restricted to just the complete states. 

In this section, we shall specifically introduce additional structure to partial states to represent partiality of the write buffers. An additional bit is added to the state structures, which can be informally interpreted as indicating whether or not the ``bottom'' of the write buffer part of the state (i.e., the end of the buffer connected to the shared memory) has been completely described, or alternatively whether additional writes may be appended later to the bottom of the buffer. Alternatively, the bottom-completeness bit can be interpreted as indicating whether or not some writes have flushed from the write buffer to the memory. 

\begin{definition}
  A \emph{partial uniprocessor memory system} is a three-tuple $(h,b,c)$, in which: 
  \begin{itemize}
    \item $h : \setlocations \pfun \setvalues$ is a heap, 
    \item $b : \listof{(\setlocations \times \setvalues)}$ is a write buffer, and 
    \item $c : \setbooleans$ is a boolean indicating bottom-completeness of the write buffer. 
  \end{itemize} Furthermore, we require that $h = \nil$ whenever $c = \bvf$. A partial uniprocessor memory system $(h,b,c)$ in which $c = \bvt$ is called \emph{complete}.  
\end{definition}

Generally, the semantics of sequential commands is essentially unchanged from the definition given in the previous chapter: each transition defined between uniprocessor memory systems is defined similarly for partial uniprocessor memory systems, with the bottom-completeness flag unchanged in each step. The only exception is with the definition of the $\taustep$ relation---and hence the \textsc{c-tau} rule---which now sets the bottom-completeness flag whenever a write is flushed to memory: \begin{align*} (h,b \lapp \llit{(\ell,v)},c) \taustep (\funup{h}{\ptup{\ell}{v}},b,\bvt) 
\end{align*} 
...
(s,\mu) \sentails P \fseqri Q \iffdef & \forall \mu_0,\mu_1 \st s,\mu_0 \sentails P \conj \mu_1 = \mu \fseq \mu_0 \onlyif  s,\mu_1 \sentails Q \end{align*} The equivalences that characterize the relationship between these implications and temporal conjunction are as follows: \begin{align*}
  P \fseq (P \fseqli Q) \sequiv & Q \\ 
  (P \fseqri Q) \fseq Q \sequiv & Q \\ 
...
In conjunction with the revised semantics of $\fbaru$, the right-side temporal implication $P \fseqri Q$ and its converse are especially useful. In particular, the formula $P \fseqric \fbaru$ (i.e., $\fbaru \fseqri P$) describes states which, if flushed, would satisfy $P$. For example, $x \fpointsto 1 \fseqric \fbaru$ describes states with any number of buffered writes to address $x$, with the final one having value 1. One consequence is that: \[(x \fwriteu 2) \fseq (x \fwriteu 1) \sentails (x \fpointsto 1) \fseqric \fbaru.\] The ability to express such sets of states will be of crucial importance to the program logic for concurrent programs described in the following chapter. 


\section{A Simplified Sequential Proof Theory}
...
We may then try to prove the following constituent specifications, which maintain the invariant $I$: \begin{align*}
  ...
  &\spec{I}{x = 0 \conj \femp}{c_r}{(x = 0 \conj \femp) \disj (x = 1 \conj d \fpointsto 1)}
