...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item $\gamma : \setbooleans$ is a boolean indicating bottom-completeness of the write buffer. 
  \end{itemize} Furthermore, we require that $h = \nil$ whenever $\gamma = \bvf$. A partial uniprocessor memory system $(h,b,\gamma)$ in which $\gamma = \bvt$ is called \emph{complete}.  
\end{definition}

The semantics of sequential commands is adapted from the definition given in the previous chapter by requiring that transitions only occur between complete uniprocessor memory systems. That is, configurations of the shape $c,(s,h,b,\bvf)$ have no transitions. 
