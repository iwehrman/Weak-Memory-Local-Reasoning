...
Graphical models are a simple but amazingly expressive formalism. Recent work has shown how they can be used to give various weak-memory semantics to concurrent programs \cite{DBLP:series/natosec/HoareW11}. They can also be used to show the soundness of rely/guarantee-style program logics \cite{DBLP:journals/ipl/WehrmanHO09}. There, assertions are modeled by sets of graphical models just the same as programs, thus leading to significantly  simplified soundness proofs. These proofs were later simplified further by modeling the program logics with algebraic characterizations of suitable graphical models called Concurrent Kleene Algebras \cite{DBLP:conf/RelMiCS/HoareMSW09,DBLP:conf/concur/HoareMSW09}. 

 Locality bimonoids \cite{DBLP:conf/concur/HoareHMOPS11} 
