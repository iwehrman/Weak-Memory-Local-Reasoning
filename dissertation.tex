...
In this section, we shall specifically introduce additional structure to partial states to represent partiality of the write buffers. An additional bit is added to the states that can be informally interpreted as indicating whether or not the ``bottom'' of the write buffer part of the state (i.e., the end of the buffer connected to the shared memory) has been completely described, or alternatively whether additional writes may be appended later to the bottom of the buffer. 

\begin{definition}
  A \emph{partial uniprocessor memory system} is a three-tuple $(h,b,c)$, in which: 
  \begin{itemize}
    \item $h$ is a heap, 
    \item $b$ is a write buffer, and 
    \item $c$ is a boolean indicating completeness of the write buffer. 
  \end{itemize} Furthermore, we require that $h = \nil$ whenever $c = \bvf$ then. A partial uniprocessor memory system $(h,b,c)$ in which $c = \bvt$ is called \emph{complete}.  
\end{definition}


\section{Partial Assertions}

\section{Simplified Sequential Proof Theory}
