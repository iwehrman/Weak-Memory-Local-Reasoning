...
\begin{lemma}
    ...
    \begin{enumerate}
        \item $S_0 \ssep S_1$ is a predicate; 
        \item $S_0 \sseq S_1$ is a predicate; and 
        \item $S_0 \shash S_1$ is a predicate.
    \end{enumerate}
\end{lemma}
...
We call to pairs $(\mu,\gamma)$ that are part of a uniprocessor model $\amod$ \emph{generalized uniprocessor memory systems}, and refer to them by the symbol $\nu$.
...
We write $\pred{P}$ for the set of all stack-model pairs that satisfy an assertion $P$,\[ \pred{P} \eqdef \setof{s,\amod}{s,\amod \sentails P},\] and also $P \sentails P'$ and $P \sequiv P'$ for semantic entailment and equivalence, respectively: \begin{align*}
...
The machine model described here is more general than the model used to describe the memory model in that it allows, via the blocked set $k$, an arbitrary subset of processors to be considered blocked. This is as opposed to the use of a lock, which indicates that either no processors are blocked (the lock is available) or all but a single processor $i$ are blocked (the lock is held by $i$). Intuitively, this can be thought of has representing partial information about the status of the system. In particular, if the blocked set contains any processor identifiers then the global lock must be held by some processor, though it may be unclear which one specifically holds the lock. A partial memory system is called \emph{complete} if all its buffers are bottom-complete and its set of blocked processors is a valid representation of a global lock---insofar as no processors are blocked when the lock is free, and all processors but $i$ are blocked when $i$ holds the lock: \[ \complete{h,B,k,\Gamma} \iffdef \Gamma = \setprocessors \conj (k = \nil \disj \exists i \in \setprocessors \st k = \setprocessors \setminus \set{i}).\] If a memory system is not (necessarily) complete, then it is called \emph{partial}.
