...
For example, the set $S_0 \eqdef \setof{(s,\nil,\llit{(\ell,v)})}{s \in \setstacks}$, which consists of states that have a single buffered write, is not closed w.r.t.\ flushing, because that write may nondeterministically commit to memory: \[(\nil,\llit{(\ell,v)}) \taustep (\ell \mapsto v,\lnil),\] according to the definition of the flushing relation $\taustep$ in Section~\ref{sec:sequential-programs}. On the other hand, the set $S_1 \eqdef \setof{(s,\ell \mapsto v,\lnil)}{s \in \setstacks}$ is closed w.r.t.\ flushing; each state in $S_1$ is completely flushed, and so can take no flushing steps. And so too is the set $S_0 \cup S_1$, because elements of $S_0$ may step to elements of $S_1$. 
