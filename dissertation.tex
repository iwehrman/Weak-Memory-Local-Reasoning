...
It remains to show that the sequential commands $c'_s$ and $c_r$ satisfy their individual specifications, each maintaining the invariant $I$ along the way. The specification of $c'_s$ follows from an application of the inference rule (\textsc{atomic}) for well-locked commands: \[ \infer[\textsc{atomic}]{\spec{I}{x = 0 \conj d \fpointsto 0}{\clock_0 \opseq c \opseq \cunlock_0}{d \fpointsto 1} }{\spec{\femp}{\flock{0} \fsep \fexp{I} \fsep (x = 0 \conj d \fpointsto 0)}{c}{\flock{0} \fsep \fexp{I} \fsep d \fpointsto 1}}\] where we abbreviate $\cstore{d}{1}_0 \opseq \cassign{s}{1}_0$ as $c$ above. Once the lock has been acquired with the $\clock$ primitive, the antecedent of this rule moves the invariant $I$ from the shared portion of state into the the thread's private state. While the lock is held the command may temporarily violate the the invariant, so long as it is repaired by the time the command has finished executing and before releasing the lock with the $\cunlock$ primitive. 

In the pre and postcondition, the assertion $\flock{0}$ ensures that in the starting and ending state, processor 0 does in fact hold the lock. When the shared invariant $I$ is moved into the threads local state, it is interpreted as the \emph{expansion} of the invariant, $\fexp{I}$. The expansion of a formula (informally) describes not just the states that satisfy $I$, but also other states which have heap parts that, as they flush to memory, would also satisfy $I$. For example, $x \fpointsto 1 \fseq x \fwrite{0} 1 \sentails \fexp{x \fpointsto 1}$, because regardless of how the buffered write on the left is flushed, the heap part of the state always satisfies the assertion $x \fpointsto 1$. 
