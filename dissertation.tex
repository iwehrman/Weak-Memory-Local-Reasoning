...
Consider, by way of example, the behavior of a load command $\cload{x}{y}$ in the context of a stack $s$ and a complete memory system $\mu_G$. Suppose further that there exists for some local memory system $\mu_L$ and frame $\mu_F$ such that $\mu_G = \mu_F \bullet \mu_L$, for some as-yet-unspecified notion of composition $\bullet$. Safety monotonicity requires that if $\cload{x}{y},(s,\mu_L) \rightarrow \top$ then also $\cload{x}{y},(s,\mu_G) \rightarrow \top$. The frame property requires additionally that if $\cload{x}{y},(s,\mu_G) \rightarrow \cskip,(\funup{s}{\ptup{x}{v}},\mu_G)$ then also $\cload{x}{y},(s,\mu_L) \rightarrow \cskip,(\funup{s}{\ptup{x}{v}},\mu_L)$. In the context of the load command, this means that the lookup function must satisfy the safety monotonicity and frame properties: for $\mu_G = (h_G,b_G)$ and $\mu_L = (h_L,b_L)$, if $(h_G \override \funof{b_G})(\ext{s}(y)) = \bot$ then $(h_L \override \funof{b_L})(\ext{s}(y)) = \bot$, and otherwise $(h_L \override \funof{b_L})(\ext{s}(y)) = (h_G \override \funof{b_G})(\ext{s}(y))$. The first condition requires that the domain of the local state be included in the domain of the global state; and the second condition requires that the lookup functions of the global and local state agree for mutually defined points.  

What might a reasonable definition be for $(\mu_F \bullet \mu_L)$? We might search for a \emph{temporal separation}, in which the writes of $\mu_F$ precede the writes of $\mu_L$ in the global state $(\mu_F \bullet \mu_L)$. A conservative definition for a temporal separation function would be to \emph{add} heaps of the respective memory systems when their domains are disjoint, and to \emph{concatenate} the write buffers: \[ (h_F,b_F) \bullet (h_L,b_L) \eqdef \begin{cases}
    (h_F \cup h_L, b_F \lapp b_L) & \text{if $b_F = \lnil \disj h_L = \nil$} \\ 
    \bot & \text{otherwise.}
\end{cases}\] Because the separation is only defined when the heap domains are disjoint, the addition of heaps does in fact result in a heap; and the concatenation of write buffers always results in a write buffer, so the separation is well defined. Furthermore, the safety monotonicity holds easily, because $\dom{h_L\override b_L} \subseteq \dom{(h_F \cup h_L) \override (b_F \lapp b_L)}$. 
