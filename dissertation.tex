...
An explicit goal is that each conjunction $\bullet$ be \emph{local} w.r.t.\ the programming language in that whenever $c,(\sigma_F \bullet \sigma) \rightarrow c',\sigma'$ for some ``frame'' state $\sigma_F$, then $c,\sigma \rightarrow c',\sigma_0$ such that $\sigma' \in (\sigma_F \bullet \sigma_0)$, or $c,\sigma \rightarrow \top$. This property allows for reasoning about the behavior of a command in a larger state to be reduced to to reasoning about the behavior of the command in a substate. 
...
\begin{enumerate}
	\item The $\fbar{e}$ assertion is additive. It would be better if it described no writes, and if we could express flushing by writing $P \fhash \fbar{e}$ instead of $P \conj \fbar{e}$. Then the fence axiom could be smaller.
\end{enumerate}
