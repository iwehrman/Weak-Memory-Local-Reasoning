...
\begin{document} 
	...
	\author{Ian Wehrman} 
	\title{Weak-Memory Local Reasoning\\Summary of Progress\footnote{This is a draft. Please do not distribute without permission.}} 
...
This section contains a brief, informal review of the x86-TSO memory model as defined by Owens, Sarkar and Sewell \cite{DBLP:conf/tphol/OwensSS09}. The model is given as a collection of legal traces of memory events. Legal traces are defined axiomatically and operationally, the latter via a labeled transition relation between machine states. These states are four-tuples $(R,m,B,l)$ in which: \begin{itemize}
	...
	\item $B : \setprocessors \tfun (\setlocations \times \setvalues)~ \mathit{list}$ is a write buffer for each processor; 
...
A machine state is a four-tuple $(s,h,B,l)$ such that: \begin{itemize}
	\item $s$ is a stack, as defined in Section~\ref{sec:expressions};
...
A \emph{concrete state} is a four-tuple $(s,h,B,l)$ such that: \begin{enumerate}
	...
	\item $h : (\setlocations \times \setvalues) \mathit{list}$ is a concrete heap; 
	\item $B : (\setprocessors \tfun (\setlocations \times \setvalues) \mathit{list})$ is an array of write buffers; and 
	\item $l : \setprocessors^\bot$ is a global lock. 
\end{enumerate}  
